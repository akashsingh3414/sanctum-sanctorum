...
def calculate_late_fee(due_at: datetime, returned_at: datetime, price_cents: int) -> int:
    ...
    days_late = math.ceil(diff.total_seconds() / 86400.0)
...
def create_loan(db: Session, data: LoanCreate, now: datetime) -> LoanOut:
    ...
    unreturned_loans = db.scalars(
        select(Loan).where(Loan.member_id == member.id, Loan.returned_at.is_(None))
    ).all()

    # Check for overdue loans (409)
    for l in unreturned_loans:
        if l.due_at and now > l.due_at:
            raise HTTPException(status_code=409, detail="Member has overdue loan")

    # Check for already borrowing same book (409)
    for l in unreturned_loans:
        if l.book_id == book.id:
            raise HTTPException(status_code=409, detail="Book already borrowed by member")

    # Check tier limit (409)
    limit = TIER_LOAN_LIMIT.get(member.tier)
    if limit is not None and len(unreturned_loans) >= limit:
        raise HTTPException(status_code=409, detail=f"Member reached loan limit for tier '{member.tier}'")

    # Check stock (409)
    if book.stock <= 0:
        raise HTTPException(status_code=409, detail="Book out of stock")
...
def list_member_loans(
    ...
    out = [to_loan_out(l, now) for l in loans]
