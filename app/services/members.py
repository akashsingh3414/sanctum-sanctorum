...
def get_member_stats(db: Session, member_id: int, now: datetime) -> MemberStats:
    ...
    active_loans = sum(1 for l in loans if l.returned_at is None)
    overdue_loans = sum(1 for l in loans if l.returned_at is None and l.due_at and now > l.due_at)
    late_fees_cents = sum(l.late_fee_cents for l in loans)
